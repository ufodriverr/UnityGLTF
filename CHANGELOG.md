...
## [Unreleased]
- change: the full-precision RGBM8 lightmap pages are no longer embedded in the GLB - each page ships as a lossless `<name>_Lightmap-<i>_RGBM8.png` sidecar (byte-identical RGBM8 encode, no tone curve, no rescale) listed in `IMMERSION_lightmaps.rgbmPages`, and the GLB only keeps a 4x4 black RGBM placeholder page per lightmap under the original texture name so `lm_index` / page ordering stay valid. `GltfCustomData.EmbedFullLightmapPages` (static) or the plugin's **Embed Full Lightmap Pages** toggle restores the old behaviour. See `Documentation~/IMMERSION_lighting.md`.
