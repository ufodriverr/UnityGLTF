...
## [Unreleased]
- change: **a scene now exports exactly ONE file per lightmap page** - the lossless `<name>_Lightmap-<i>_RGBM8.png` sidecar. The legacy tone-curve LDR page (`<name>_Lightmap-<i>.png`) is no longer written; `IMMERSION_lightmaps`' **Embed Textures In Glb** toggle still produces that clamped LDR decode as an embedded glTF texture for non-Immersion consumers (and is now the only consumer of the plugin's Lightmap Texture Scale / Max Size settings). The `<name>_lightmap_offsets.json` manifest is unchanged - its `colorName` (`<name>_Lightmap-<i>`) prefix-matches the RGBM8 page.
- change: `IMMERSION_lightmaps` root extension payload is **version 2**: `lightmaps[].image` now names the RGBM8 page as an already-**resolved** file name (no `{name}` token), and the separate `rgbmPages` array is gone - read `lightmaps[].image`. The node-level `IMMERSION_lightmap.image` is resolved the same way. Consumers that read `rgbmPages` or expect an LDR `image` must be updated.
- add: `GLTFSceneExporter.SidecarBaseName` - the base name `{name}` sidecar tokens resolve to, set before the export plugins run so they can bake resolved sidecar file names into glTF extensions (the glTF JSON is never token-replaced).
- change: the full-precision RGBM8 lightmap pages are not embedded in the GLB - each page ships as a lossless `<name>_Lightmap-<i>_RGBM8.png` sidecar (byte-identical RGBM8 encode, no tone curve, no rescale) named in `IMMERSION_lightmaps.lightmaps[].image`, and the GLB only keeps a 4x4 black RGBM placeholder page per lightmap under the original texture name so `lm_index` / page ordering stay valid. `GltfCustomData.EmbedFullLightmapPages` (static) or the plugin's **Embed Full Lightmap Pages** toggle restores the old behaviour. See `Documentation~/IMMERSION_lighting.md`.
- add: `IMMERSION_lightmaps` export plugin (enabled by default) - writes the `<name>_lightmap_offsets.json` manifest in the Immersion web editor schema next to the exported file and adds `IMMERSION_lightmap`/`IMMERSION_lightmaps` extensions to the glTF (the page pixels come from the Gltf Custom Shaders Export plugin's RGBM8 sidecars). See `Documentation~/IMMERSION_lighting.md`.
...
- fix: materials from unknown custom shaders (exported via the `_BaseMap`/`_ColorTexture` fallback) now write `metallicFactor 0` / `roughnessFactor 1` instead of inheriting the glTF defaults (metallic 1), which rendered them as dark metal in viewers.
- add: custom shader export by property-name matching - albedo (Albedo/Main/Color/Base/Diffuse + Map/Texture/Tex/Transparency suffixes, with or without `_` prefix, case-insensitive) and normals (Normal/Bump + suffixes) now export from any custom/ShaderGraph/Amplify shader without editing materials. Base color factor is matched the same way (Color/BaseColor/Tint/...).
- add: separate `Lightmap Texture Scale` / `Lightmap Max Texture Size` settings on the `IMMERSION_lightmaps` plugin (defaults: full resolution, no cap) - lightmaps no longer follow the global texture scale/cap.
- change: lightmap sidecar PNGs are now prefixed with the export name (`Bank_Lightmap-0.png` instead of `Lightmap-0.png`), and the offsets JSON `colorName` follows - lightmaps from different scenes no longer overwrite each other in a shared folder/asset store.
- fix: specular-glossiness materials now also write an approximate `pbrMetallicRoughness` fallback (metallic 0, roughness = 1 - glossiness, diffuse as base color) for viewers without `KHR_materials_pbrSpecularGlossiness` support (e.g. current three.js).
